function dydt = odefunc(t,y,c_forces, c_moments,...
    a_forces,a_moments,flag,controlled)
...
if flag
    ...
    % Uncomment this to add random perturbation
    % if(t > 5 && t < 6)
    %     y(11) = y(11) + 5;
    % end
    ...
    g_b = g*[-sin(theta); sin(phi)*cos(theta); cos(phi)*cos(theta)];

    accel = g_b + (1/m)*a_forces + (1/m)*(c_forces) - ...
    ...
    if controlled
        dydt(4) = (r/(sqrt(2)*I_x))*(0.004*-dev_p);
        dydt(5) = (r/(sqrt(2)*I_y))*(0.004*-dev_q);
        dydt(6) = (k/(I_z))*(0.004*-dev_r);
    else
