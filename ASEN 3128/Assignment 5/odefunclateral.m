function dydt = odefunclateral(t,y,c_forces, c_moments,flag,controlled,dev_v_ref,...
    dev_u_ref)
...
if flag
    ...
    I_x = 5.8*(10^-5); % moment of inertia [kg m^2]
    I_y = 7.2*(10^-5); % moment of inertia [kg m^2]
    I_z = 1.*(10^-5); % moment of inertia [kg m^2]
    ...
    if controlled
        dydt(4) = (-k1*dev_p -k2*dev_phi - k2*k3*dev_v_E)/(I_x) +...
            (k2*k3*dev_v_ref/I_x);% change in roll rate [rad/s^2]
        dydt(5) = 0*((-k4*dev_q -k5*dev_theta - k5*k6*dev_u_E)/(I_y) +...
            (k5*k6*dev_u_ref/I_y));% change in pitch rate [rad/s^2]
        dydt(6) = 0*-0.004*dev_r/I_z;% change in yaw rate [rad/s^2]
    else
