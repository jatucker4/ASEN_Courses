...
legend('2D Wing','3D Wing')
%% Swet estimation
wing_wet = 2*1.25*0.9*.1;
wing_wet_total = 2*wing_wet;
tail_wet_total = 2*1.25*.2*.04;
fuselage_length = 0.45;
fuselage_radius = .025;
...
alpha_L_0 = alpha_2d(1,3) + (0 - CL_3d(1,3))*(alpha_2d(1,4)-alpha_2d(1,3))/(CL_3d(1,4)-CL_3d(1,3));

x = alpha_2d(1,3):alpha_2d(1,10);
y = CL_3d(1,3:10);
p = polyfit(x,y,1);
a_0 = p(1);
...
legend('Whole Aircraft','Tempest UAS')
...
h = 80; % (m)
GTOW = 0.0528*9.81;
rho = 1.0581 + (1.8 - 1.5)*((1.0066 - 1.0581)/(2 - 1.5));
