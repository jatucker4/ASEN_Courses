function [joints,connectivity,reacjoints,reacvecs,loadjoints,...
    loadvecs]=readinput(inputfile)
% function [joints,connectivity,reacjoints,reacvecs,
...
while line > 0
    ...
    switch inpblk
        ...
        case 1 % read number of joints, bars, reactions, and loads
            ...
            if numreact~=6; error('incorrect number of reaction forces');
            end
        ...
        case 2 % read coordinates of joints
            ...
            if jointid>numjoints || jointid<1
                error(strvcat({'joint id number need to be',...
                    'smaller than number',...
                    'of joints and larger than 0'}));
            end
        ...
        case 3 % read connectivity of bars
            ...
            if barid>numbars || barid<0
                error(strvcat({'bar id number needs to be smaller than'...
                    'number of bars and larger than 0'}));
            end
            
            % check joint ids
            if max(tmp(2:3))>numjoints || min(tmp(2:3))<1
               error(strvcat({'joint id numbers need to be smaller than',...
                    'number of joints and larger than 0'}));
            end
        ...
        case 4 % read reation force information
            ...
            jointid=tmp(1);
            if jointid>numjoints || jointid<1
                error(strvcat({'joint id number need to be smaller than',...
                    'number of joints and larger than 0'}));
            end
