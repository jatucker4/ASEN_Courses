...
% read input file
[joints,connectivity,reacjoints,reacvecs,loadjoints,...
    loadvecs]=readinput(inputfile);

% compute forces in bars and reactions
[barforces,reacforces]=forceanalysis(joints,connectivity,...
    reacjoints,reacvecs,loadjoints,loadvecs);

% write outputfile
writeoutput(outputfile,inputfile,barforces,reacforces,joints,...
    connectivity,reacjoints,reacvecs,loadjoints,loadvecs);

% plot truss (used in Lab 2)
joints3D=zeros(size(joints,1),3);
joints3D(:,1:3)=joints;
plottruss(joints3D,connectivity,barforces,...
    reacjoints,3*[0.025,0.04,0.05],[1 1 0 0])
